...
from cicerone.config import DatabaseLocation, DatasetLocation, IOSettings
...
def test_build_input_source_dataset(tmp_path):
    settings = IOSettings(kind="dataset", dataset=DatasetLocation(backend="local", path=str(tmp_path)), database=None)
...
def test_build_input_source_db():
    settings = IOSettings(kind="db", dataset=None, database=DatabaseLocation(url="postgresql+psycopg://u:p@h/d"))
...
def test_build_input_source_unknown_kind_raises():
    settings = IOSettings(kind="carrier-pigeon", dataset=None, database=None)
