...
interactions exist, intra-list diversity when item features exist, serendipity
when prior interactions exist, and hit rate by `source`). This is not CTR.
Recommendation snapshots (`recommendation_history`) let the window span more
than one job.
