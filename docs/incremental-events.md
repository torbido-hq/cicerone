...
`max_poll_interval_ms` must be >= `session_timeout_ms`). Setting
`session_timeout_ms` also sets `heartbeat.interval.ms` to one third of the
session so a short session is not paired with the 3s client default.
Missing `event_id`
