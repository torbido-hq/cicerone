...
session. `session_timeout_ms` must be >= 2 and <= 3600000 (librdkafka
`session.timeout.ms`); `max_poll_interval_ms` must be <= 86400000.
Setting `session_timeout_ms` also sets `heartbeat.interval.ms` to one third
of the session so a short session is not paired with the 3s client default.
Missing `event_id`
...
deduped by event fingerprint. `nack` calls `basic_nack(requeue=True)` so
prefetch is released and the broker redelivers. AMQP calls run on one I/O
thread; `heartbeat`
